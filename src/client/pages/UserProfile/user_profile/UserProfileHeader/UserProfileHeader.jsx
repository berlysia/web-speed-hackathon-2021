...
import { FontAwesomeIcon } from '../../../../components/foundation/FontAwesomeIcon';
  ...
  return (
    <header className="relative">
      ...
      <div className="absolute left-2/4 m-0 w-28 h-28 bg-gray-300 border border-gray-300 rounded-full overflow-hidden transform -translate-x-1/2 -translate-y-1/2 sm:w-32 sm:h-32">
        <img alt="" crossOrigin="anonymous" onLoad={handleLoadImage} src={getProfileImagePath(user.profileImage.id)} srcSet={`${getProfileImagePath(user.profileImage.id, "256")} 256w, ${getProfileImagePath(user.profileImage.id, "128")} 128w`} />
      </div>
      <div className="pt-20 px-4">
        ...
        <p className="pt-2 text-gray-600 text-sm">
          ...
          <span>
            <time dateTime={formatDateToISO(user.createdAt)}>{formatDate(user.createdAt)}</time>
            からサービスを利用しています
          </span>
        </p>
      </div>
